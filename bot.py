...
async def main():
    ...
    user_client = Client(
        ...
        api_id=int(API_ID),
        api_hash=API_HASH,
        session_string=USERBOT_SESSION_STRING,  # String de sesión del userbot
        parse_mode=ParseMode.HTML,  # Configurar parse_mode por defecto
        no_updates=True  # No necesitamos actualizaciones para el userbot
    )
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    logger.critical(f"Error iniciando UserBot: {str(e)}")
                    logger.critical("Verifica que USERBOT_SESSION_STRING sea válido")
                    raise
            ...
            logger.critical(f"Error inesperado en la función principal: {e}", exc_info=True)
            raise
