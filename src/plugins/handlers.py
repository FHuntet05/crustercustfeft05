...
async def start_command(client: Client, message: Message):
    ...
    start_message = (f"A sus órdenes, {greeting}, bienvenido a la <b>Suite de Medios v16.1 (Estable)</b>.\n\n"
                     ...
                     "• /p clean - Limpia todas las tareas.\n"
                     "• /profiles - Gestiona sus perfiles guardados.\n"
                     "• /join - Une varios videos del panel.\n"
                     "• /zip - Comprime tareas en un archivo ZIP.\n"
...
async def panel_command(client: Client, message: Message):
    ...
    if not pending_tasks: return await message.reply(f"✅ ¡{greeting}, su mesa de trabajo está vacía!")
    response = [f"📋 <b>{greeting}, su mesa de trabajo actual:</b>"]
    for i, task in enumerate(pending_tasks): response.append(format_task_details_rich(task, i + 1))
...
async def process_command(client: Client, message: Message):
    ...
    if len(parts) < 2: return await message.reply("Uso: `/p [ID]` o `/p clean`.", parse_mode=ParseMode.MARKDOWN)
    action = parts[1].lower()
    if action == "clean": return await message.reply("¿Seguro que desea eliminar TODAS las tareas?", reply_markup=build_confirmation_keyboard("panel_delete_all_confirm", "panel_delete_all_cancel"))
    if not action.isdigit(): return await message.reply("El ID debe ser un número. Use `/panel`.")
    task_index = int(action) - 1
    pending_tasks = await db_instance.get_pending_tasks(user_id)
    if 0 <= task_index < len(pending_tasks): await processing_handler.open_task_menu_from_p(client, message, str(pending_tasks[task_index]['_id']))
    else: await message.reply(f"❌ ID inválido. Tiene {len(pending_tasks)} tareas.")

@Client.on_message(filters.command("profiles") & filters.private)
async def profiles_command(client: Client, message: Message):
    user_id = message.from_user.id; presets = await db_instance.get_user_presets(user_id)
    await message.reply("<b>Gestión de Perfiles:</b>\nAquí puede eliminar perfiles guardados.", reply_markup=build_profiles_management_keyboard(presets), parse_mode=ParseMode.HTML)

@Client.on_message(filters.command("join") & filters.private)
async def join_command(client: Client, message: Message):
    user_id = message.from_user.id; video_tasks = await db_instance.get_pending_tasks(user_id, file_type_filter="video")
    ...
    await message.reply("🎬 <b>Modo de Unión</b>\nSeleccione los videos que desea unir en orden:", reply_markup=build_join_selection_keyboard(video_tasks, []), parse_mode=ParseMode.HTML)

@Client.on_message(filters.command("zip") & filters.private)
async def zip_command(client: Client, message: Message):
    user_id = message.from_user.id; tasks = await db_instance.get_pending_tasks(user_id)
    if not tasks: return await message.reply("❌ Su panel está vacío.")
    await db_instance.set_user_state(user_id, "selecting_zip_files", data={"selected_ids": []})
    await message.reply("📦 <b>Modo de Compresión</b>\nSeleccione las tareas a incluir en el .zip:", reply_markup=build_zip_selection_keyboard(tasks, []), parse_mode=ParseMode.HTML)

@Client.on_message(filters.command("p_all") & filters.private)
async def process_all_command(client: Client, message: Message):
    user_id = message.from_user.id; tasks = await db_instance.get_pending_tasks(user_id)
    if not tasks: return await message.reply("❌ No hay tareas pendientes para procesar.")
...
async def media_gatekeeper(client: Client, message: Message):
    ...
    if user_state.get("status") != "idle": return await processing_handler.handle_media_input_for_state(client, message, user_state)
    ...
    metadata = {}; file_name = sanitize_filename(getattr(media, 'file_name', f"{file_type}_{datetime.utcnow().timestamp()}"))
    ...
    task_id = await db_instance.add_task(user_id=user_id, file_type=file_type, file_name=file_name, file_id=media.file_id, status=status, metadata=metadata)
    ...
    if status == "pending_processing" and (presets := await db_instance.get_user_presets(user_id)):
...
async def text_gatekeeper(client: Client, message: Message):
    ...
    user_state = await db_instance.get_user_state(user_id)
    if user_state.get("status") != "idle": return await processing_handler.handle_text_input_for_state(client, message, user_state)
    if re.search(URL_REGEX, text): return await handle_url_input(client, message, text)
...
async def handle_url_input(client: Client, message: Message, url: str):
    ...
    try:
        ...
        if not info: raise ValueError("No pude obtener información.")
...
async def handle_music_search(client: Client, message: Message, query: str):
    ...
    docs = [{'search_id': search_id, 'created_at': datetime.utcnow(), **res} for res in results]; await db_instance.search_results.insert_many(docs)
...
async def set_download_format_callback(client: Client, query: CallbackQuery):
    ...
    parts = query.data.split("_"); temp_info_id, format_id = parts[2], "_".join(parts[3:])
    ...
    info = info_doc['data']; file_type = 'audio' if 'audio' in format_id or 'mp3' in format_id else 'video'
    task_id = await db_instance.add_task(user_id=query.from_user.id, file_type=file_type, file_name=sanitize_filename(info['title']), url=info['url'], processing_config={"download_format_id": format_id}, url_info=info, status="queued")
