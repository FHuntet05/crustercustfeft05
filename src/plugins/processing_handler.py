...
from src.helpers.keyboards import (build_back_button, build_processing_menu,
                                   ...
                                   build_profiles_management_keyboard, build_profile_delete_confirmation_keyboard)
...
# --- Máquina de Estado: Procesadores de Entrada ---
...
async def handle_media_input_for_state(client: Client, message: Message, user_state: dict):
    ...
    state_map = {
        "awaiting_audiothumb": "thumbnail_file_id", "awaiting_subs": "subs_file_id",
        "awaiting_watermark_image": "watermark", "awaiting_thumbnail_add": "thumbnail_file_id",
...
# --- Enrutadores de Callbacks (por Prefijo) ---
...
# --- Implementaciones de Lógica de Callback ---
...
async def show_config_menu_and_set_state(client: Client, query: CallbackQuery):
    ...
    state_map = { "rename": "awaiting_rename", "trim": "awaiting_trim", "split": "awaiting_split", "gif": "awaiting_gif", "audiotags": "awaiting_audiotags", "audiothumb": "awaiting_audiothumb", "addsubs": "awaiting_subs", "thumbnail_add": "awaiting_thumbnail_add", "replace_audio": "awaiting_replace_audio", "watermark_text": "awaiting_watermark_text", "watermark_image": "awaiting_watermark_image" }
    ...
    menu_texts = { "rename": f"✏️ {greeting}, envíe el nuevo nombre.", "trim": f"✂️ {greeting}, envíe tiempos de corte (<code>INICIO-FIN</code>).", "split": f"🧩 {greeting}, envíe criterio de división (<code>300s</code>).", "gif": f"🎞️ {greeting}, envíe duración y FPS (<code>5 15</code>).", "audiotags": f"✍️ {greeting}, envíe nuevos metadatos (<code>Título: X</code>).", "audiothumb": f"🖼️ {greeting}, envíe la carátula.", "addsubs": f"➕ {greeting}, envíe el archivo <code>.srt</code>.", "thumbnail_add": f"🖼️ {greeting}, envíe la nueva miniatura.", "replace_audio": f"🎼 {greeting}, envíe el nuevo audio.", "watermark_text": f"💧 {greeting}, envíe el texto de la marca.", "watermark_image": f"🖼️ {greeting}, envíe la imagen de la marca." }
    back_callbacks = { "audiotags": "config_audiometadata_", "audiothumb": "config_audiometadata_", "addsubs": "config_tracks_", "thumbnail_add": "config_thumbnail_", "replace_audio": "config_tracks_", "watermark_text": "config_watermark_", "watermark_image": "config_watermark_" }
...
async def set_value_callback(client: Client, query: CallbackQuery):
    ...
    if config_type == "watermark" and parts[3] == "position":
        ...
        if config_type == "transcode":
            value = "_".join(parts[3:]); await db_instance.tasks.update_one({"_id": ObjectId(task_id)}, {"$unset": {"processing_config.transcode": ""}}) if value == "remove_all" else await db_instance.update_task_config(task_id, f"transcode.{value.split('_', 1)[0]}", value.split('_', 1)[1])
...
async def handle_profile_actions(client: Client, query: CallbackQuery):
    user_id = query.from_user.id
    action, parts = query.data.split("_")[1], query.data.split("_")

    if action == "save":
        task_id = parts[2]
        await db_instance.set_user_state(user_id, "awaiting_profile_name", data={"task_id": task_id, "source_message_id": query.message.id})
        await query.message.edit_text("✏️ Envíe un nombre para este perfil:", reply_markup=build_back_button(f"p_open_{task_id}"))
    elif action == "apply":
        ...
        await query.message.edit_text("✅ Perfil eliminado.\n\n<b>Sus perfiles:</b>", reply_markup=build_profiles_management_keyboard(presets), parse_mode=ParseMode.HTML)
...
async def handle_batch_actions(client: Client, query: CallbackQuery):
    ...
    if profile_id != "default":
        ...
        if not preset: return await query.message.edit_text("❌ Perfil no encontrado.")
        config_to_apply = preset.get('config_data', {})
...
async def handle_join_actions(client: Client, query: CallbackQuery):
    ...
    if action == "select":
        ...
        await query.message.edit_text("Seleccione los videos para unir:", reply_markup=build_join_selection_keyboard(tasks, selected_ids))
    elif action == "confirm":
        if len(selected_ids) < 2: return await query.answer("❌ Debe seleccionar al menos 2 videos.", show_alert=True)
        await db_instance.add_task(user_id, 'join_operation', file_name=f"Join_{len(selected_ids)}videos.mp4", status="queued", custom_fields={"source_task_ids": [ObjectId(tid) for tid in selected_ids]})
...
async def handle_zip_actions(client: Client, query: CallbackQuery):
    ...
    if action == "select":
        ...
        tasks = await db_instance.get_pending_tasks(user_id)
        await query.message.edit_text("Seleccione archivos para comprimir:", reply_markup=build_zip_selection_keyboard(tasks, selected_ids))
    elif action == "confirm":
        if not selected_ids: return await query.answer("❌ No ha seleccionado archivos.", show_alert=True)
        await db_instance.add_task(user_id, 'zip_operation', file_name=f"Zip_{len(selected_ids)}files.zip", status="queued", custom_fields={"source_task_ids": [ObjectId(tid) for tid in selected_ids]})
...
async def select_song_from_search(client: Client, query: CallbackQuery):
    ...
    if not url_info or not url_info.get('url'):
        return await query.message.edit_text("❌ No pude encontrar una fuente de audio descargable.")

    task_id = await db_instance.add_task(user_id=query.from_user.id, file_type='audio', file_name=f"{search_result['artist']} - {search_result['title']}", url=url_info['url'], status="queued",
        processing_config={"download_format_id": downloader.get_best_audio_format_id(url_info.get('formats', [])), "audio_tags": {'title': search_result['title'], 'artist': search_result['artist'], 'album': search_result.get('album')}},
        url_info=url_info
    )

    if thumbnail_url := search_result.get('thumbnail'):
        thumb_path = os.path.join(DOWNLOAD_DIR, f"thumb_{task_id}.jpg")
        if await asyncio.to_thread(downloader.download_thumbnail, thumbnail_url, thumb_path):
            await db_instance.update_task_config(task_id, "thumbnail_path", thumb_path)
