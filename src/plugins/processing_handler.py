...
async def handle_media_input_for_state(client: Client, message: Message, user_state: dict):
    ...
    if state in ["awaiting_watermark_image", "awaiting_thumbnail_add"]:
...
# --- Manejadores de Lógica de Callback ---
...
async def show_config_menu_and_set_state(client: Client, query: CallbackQuery):
    ...
    keyboards = {"transcode": build_transcode_menu(task_id), "tracks": build_tracks_menu(task_id, config),
                 "watermark": build_watermark_menu(task_id), "thumbnail": build_thumbnail_menu(task_id, config)}
    menu_messages = {"transcode": "📉 Seleccione resolución:", "tracks": "📜 Gestione pistas:",
                     "watermark": "💧 Añada marca de agua:", "thumbnail": "🖼️ Gestione miniatura:"}
    ...
    menu_texts = {
        ...
        "thumbnail_add": "🖼️ Envíe la nueva imagen para la miniatura.",
        "replace_audio": "🎼 Envíe el archivo de audio de reemplazo.",
        "watermark_text": "💧 Envíe el texto para la marca de agua.",
        "watermark_image": "🖼️ Envíe la imagen para la marca de agua.",
        "profile_save_request": "✏️ Envíe un nombre para este perfil."
    }
    
    back_callbacks = {"addsubs": "config_tracks_", "thumbnail_add": "config_thumbnail_",
...
async def handle_batch_actions(client: Client, query: CallbackQuery):
    ...
    if action == "apply" and len(parts) > 2:
        preset = await db_instance.get_preset_by_id(parts[2])
        if preset: config_to_apply = preset.get('config_data', {})
