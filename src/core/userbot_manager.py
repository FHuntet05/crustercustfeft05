...
class UserbotManager:
    ...
    async def start(self):
        """Inicia el cliente Pyrogram si está configurado."""
        if not all([self.api_id, self.api_hash, self.session_string]):
            logger.warning("[USERBOT] Faltan API_ID, API_HASH o SESSION_STRING. El Userbot no se iniciará. Las descargas > 20MB fallarán.")
            return
        ...
        try:
            ...
            logger.critical(f"[USERBOT] ¡Error CRÍTICO de autenticación! La SESSION_STRING es inválida o ha sido revocada. Error: {e}")
            self.client = None
    ...
    async def download_file(self, file_id: str, download_path: str, progress_callback=None):
        ...
        # Pyrogram usa 'message' para referirse a la fuente, que puede ser un file_id
